...
export const ALL_COUNTRIES: Location[] = [
...
/** Google Geocoding sometimes returns long names; map to our list. */
const COUNTRY_NAME_ALIASES: Record<string, Location> = {
  'united states of america': { name: 'United States', code: 'USA' },
  'russian federation': { name: 'Russia', code: 'RUS' },
  'united kingdom of great britain and northern ireland': { name: 'United Kingdom', code: 'GBR' },
  'republic of korea': { name: 'Korea, South', code: 'KOR' },
  "democratic people's republic of korea": { name: 'Korea, North', code: 'PRK' },
  'iran (islamic republic of)': { name: 'Iran', code: 'IRN' },
  'viet nam': { name: 'Vietnam', code: 'VNM' },
  "lao people's democratic republic": { name: 'Laos', code: 'LAO' },
  'democratic republic of the congo': { name: 'Congo, Democratic Republic', code: 'COD' },
  'türkiye': { name: 'Turkey', code: 'TUR' },
  'bolivia (plurinational state of)': { name: 'Bolivia', code: 'BOL' },
  'venezuela (bolivarian republic of)': { name: 'Venezuela', code: 'VEN' },
  'tanzania, united republic of': { name: 'Tanzania', code: 'TZA' },
  'syrian arab republic': { name: 'Syria', code: 'SYR' },
  'republic of moldova': { name: 'Moldova', code: 'MDA' },
  'ivory coast': { name: "Côte d'Ivoire", code: 'CIV' },
  'côte d\'ivoire': { name: "Côte d'Ivoire", code: 'CIV' },
};

export function findCountryByName(geocodeName: string): Location | undefined {
  const normalized = geocodeName.trim().toLowerCase();
  if (!normalized) return undefined;
  const alias = COUNTRY_NAME_ALIASES[normalized];
  if (alias) return alias;
  const exact = ALL_COUNTRIES.find((c) => c.name.toLowerCase() === normalized);
  if (exact) return exact;
  return ALL_COUNTRIES.find((c) => normalized.includes(c.name.toLowerCase()) || c.name.toLowerCase().includes(normalized));
}
